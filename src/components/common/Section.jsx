...
const Section = memo(
  ({
    ...
    const renderContent = () => {
      if (type === "basic") {
        return (
          <div className="glass h-12 w-full flex items-center justify-center rounded px-4 text-xs text-white sm:text-sm md:text-base glass-hover">
            {content}
          </div>
        );
      }
        ...
        return (
          <div className="flex flex-col sm:flex-row gap-3">
            {progressData.map((item, index) => (
              <div
                key={index}
                className="glass flex w-full flex-col gap-2 rounded p-2.5 text-white sm:w-1/2 glass-hover"
              >
